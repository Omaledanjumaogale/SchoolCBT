...
function createCBTStore() {
  ...
  return {
    subscribe,
    setSubject: (subject: string, examType: ExamType) =>
      update(s => ({ ...s, subject, examType })),
    ...
    prevQuestion: () =>
      update(s => ({ ...s, currentQ: Math.max((s.currentQ ?? 0) - 1, 0) })),
    jumpTo: (i: number) => update(s => ({ ...s, currentQ: i })),
    tickTimer: () =>
      update(s => ({ ...s, timeLeft: Math.max((s.timeLeft ?? 0) - 1, 0) })),
    reset: () => set(initial)
  };
}
