...
export function subscribeQuery<T>(
  ...
  const subscription = convex.onUpdate(queryName, args, (result: T) => {
    onResult(result);
  });
...
export async function queryOnce<T>(queryName: string, args: Record<string, unknown> = {}): Promise<T | null> {
  if (!browser) return null;
  const convex = getConvexClient();
  return await convex.query(queryName, args) as T;
}

// Helper: execute a mutation
export async function executeMutation(mutationName: string, args: Record<string, unknown> = {}): Promise<any> {
  if (!browser) return null;
  const convex = getConvexClient();
  return await convex.mutation(mutationName, args);
}
