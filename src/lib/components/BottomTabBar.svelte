<script lang="ts">
  ...
  import { isAuthenticated, showModal } from '$lib/stores'
  ...
  const authTabs = $derived([
    { href: '/pricing', icon: '💰', label: 'Pricing' },
    {
      ...
      onClick: loggedIn ? undefined : () => showModal('login'),
    },
...
<nav
  class="md:hidden fixed bottom-0 left-0 right-0 z-40 glass-nav border-t border-white/06 flex items-center justify-around py-1 px-1"
  style="padding-bottom:max(0.25rem, env(safe-area-inset-bottom))"
>
  {#each tabs as tab}
    <a
      href={tab.href}
      class="flex flex-col items-center justify-center gap-0.5 min-h-[50px] min-w-[44px] px-1 py-1 rounded-lg transition-colors {isActive(
        tab.href,
      )
