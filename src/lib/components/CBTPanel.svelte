<script lang="ts">
  ...
  const questions = [
    {
      q: 'A body of mass 5 kg accelerates at 2 m/s². Net force =',
    ...
    },
    {
      ...
      opts: ['Velocity', 'Force', 'Speed', 'Displacement'],
      correct: 2,
      bloom: 'Knowledge',
    },
    {
    ...
    {
      q: 'Wave frequency 5 Hz, wavelength 2 m. Speed =',
      opts: ['5 m/s', '2 m/s', '10 m/s', '0.4 m/s'],
      correct: 2,
      bloom: 'Application',
    },
    {
      q: 'SI unit of electric charge:',
      opts: ['Ampere', 'Watt', 'Coulomb', 'Volt'],
      correct: 2,
      bloom: 'Knowledge',
    },
  ]
  ...
  function nextQ() {
    ...
    if (currentQ < questions.length - 1) {
      ...
      clearTimerInterval()
      feedback = `🎉 Done! Score: ${score}/${questions.length * 2}`
    }
  }
...
<div class="glass-card overflow-hidden" style="border-color:rgba(255,215,0,0.2)">
  ...
  <div class="flex items-center justify-between gap-3 border-b border-white/05 px-5 py-3">
    <div class="flex items-center gap-2.5">
      <div class="timer-ring">
        ...
        <span class="absolute font-mono font-bold text-white" style="font-size:.6rem"
          >{timeLeft}</span
        >
      </div>
      <div>
    ...
    <div class="flex flex-1 flex-col items-end gap-1">
      ...
      <div class="pbar w-full">
        <div
          class="pfill bg-gradient-to-r from-gold to-gold/70"
  ...
  <div class="p-5">
    {#if feedback}
      <div
        class="mb-4 rounded-xl p-3 text-xs font-semibold {feedbackOk
    ...
    <div class="space-y-2 mb-4">
      {#each q?.opts ?? [] as opt, i}
        ...
        <button
          ...
          disabled={isAnswered}
          class="opt-btn w-full flex items-center gap-3 p-3 text-left min-h-[44px] {isCorrect
            ? 'correct'
            : ''} {isWrongPick ? 'wrong' : ''}"
        >
          <span
            class="flex h-7 w-7 items-center justify-center rounded-lg bg-cobalt/60 text-xs font-bold text-gold shrink-0"
            >{labels[i]}</span
          >
  ...
  <div class="border-t border-white/05 px-5 py-3 flex items-center justify-between">
    <div class="flex items-center gap-2 text-xs text-white/30">
      <span class="pulse-dot h-1.5 w-1.5 rounded-full bg-jade inline-block"></span>AI · Nigerian
      Curriculum
    </div>
    <button
      onclick={() => showModal('signup')}
      class="text-xs text-gold hover:text-gold/80 font-medium">Full Platform →</button
    >
  </div>
</div>
