<script lang="ts">
  ...
  const features = [
    { icon:'🎯', title:'Intelligent CBT Engine', desc:'50-question batches dynamically generated per subject. 50s timer, ✅/❌ feedback, 2-point scoring.', prog:{ pct:87, color:'jade', label:'Pass Probability' } },
    { icon:'📊', title:'AI Performance Analytics', desc:'Predictive pass probability, topic-level breakdown, Bloom\'s Taxonomy tagging, and hour tracking.' },
...
<section id="features" class="bg-mesh relative overflow-hidden py-24">
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
      {#each features as f}
        <FeatureCard icon={f.icon} title={f.title} desc={f.desc} prog={f.prog} />
      {/each}
    </div>
  </div>
</section>
