...
const WAEC_SCALE: Array<{
  ...
  },
  {
    ...
    passed: true,
    points: 3,
    description: 'Merit — Good performance',
  },
  {
    min: 60,
    grade: 'C4',
    ...
    passed: true,
    points: 4,
    description: 'Credit — Satisfactory performance',
  },
  {
    min: 55,
    grade: 'C5',
    ...
    passed: true,
    points: 5,
    description: 'Credit — Average performance',
  },
  {
    min: 50,
    grade: 'C6',
    ...
    passed: true,
    points: 6,
    description: 'Credit — Minimum credit standard',
  },
  {
    min: 45,
    grade: 'D7',
    ...
    passed: false,
    points: 7,
    description: 'Pass — Below credit standard',
  },
  {
    min: 40,
    grade: 'E8',
    ...
    passed: false,
    points: 8,
    description: 'Pass — Minimum pass standard',
  },
  {
    min: 0,
    grade: 'F9',
    ...
    passed: false,
    points: 9,
    description: 'Fail — Below acceptable standard',
  },
]
...
export function scoreToWAEC(percentageScore: number): WAECGrade {
  return getWAECGrade(percentageScore).grade
}

export function computeGrade(percentageScore: number): GradeResult {
  return getWAECGrade(percentageScore)
}
...
export function calculatePassProbability(
  ...
  if (!recentScores.length) {
    return {
      probability: 0,
      trend: 'stable',
      recommendation: 'Start practicing to see your pass probability.',
      riskLevel: 'high',
    }
  }
  ...
  if (recentScores.length >= 2) {
    const oldAvg =
      recentScores.slice(0, -3).reduce((a, b) => a + b, 0) / Math.max(1, recentScores.length - 3)
  ...
  const recommendation =
    probability >= 80
      ? 'Excellent! Maintain your current pace and focus on remaining weak topics.'
      : probability >= 65
        ? 'Good progress! Push harder on weak areas to exceed the credit threshold.'
        : probability >= 50
          ? 'You are close to the pass mark. Increase daily practice batches to 2–3 per day.'
...
export function getStarRating(percentage: number): number {
  return calculateStarRating(percentage)
}
