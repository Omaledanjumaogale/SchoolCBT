...
<svelte:head>
  ...
  <meta
    name="description"
    content="SchoolCBT tutor dashboard. Manage your sessions, track earnings, view student performance. Join 240+ verified tutors."
  />
    ...
    {:else}
      <div class="mb-10 text-center">
        ...
        <p class="mx-auto max-w-2xl text-white/50">
          A complete tutor management system powered by Convex real-time data.
        </p>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4 md:grid-cols-4 mb-8">
        ...
        <div class="stat-card p-5 text-center">
          <div class="font-sora text-2xl font-extrabold text-gold">
            ₦{totalEarnings.toLocaleString()}
          </div>
          <div class="mt-1 font-mono text-xs text-white/35 uppercase tracking-widest">
            Total Earnings
          </div>
          <div class="badge badge-jade mt-2">Convex tracked</div>
        </div>
        <div class="stat-card p-5 text-center">
          <div class="font-sora text-2xl font-extrabold text-white">
            ₦{totalAvailable.toLocaleString()}
          </div>
          <div class="mt-1 font-mono text-xs text-white/35 uppercase tracking-widest">
            Available
          </div>
          <div class="badge badge-blue mt-2">
            {totalPending > 0 ? `₦${totalPending.toLocaleString()} pending` : 'All cleared'}
          </div>
        </div>
        <div class="stat-card p-5 text-center">
          ...
          <div class="badge badge-jade mt-2">
            {tutorData?.verified ? 'Verified ✓' : 'Pending review'}
          </div>
        </div>
      </div>
      ...
      <div class="glass-card p-6 mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <div class="font-sora text-3xl font-extrabold text-gold">
              ₦{totalEarnings.toLocaleString()}
            </div>
            <div class="text-sm text-white/45 mt-1">
              ₦{totalAvailable.toLocaleString()} available · {totalPending > 0
                ? `₦${totalPending.toLocaleString()} pending`
                : 'All cleared'}
            </div>
          </div>
          <div class="flex gap-3">
            <button class="btn-gold py-3 px-6 text-sm" disabled={totalAvailable === 0}
      ...
      <div class="glass-card p-6">
        ...
        <div class="text-center py-12 text-white/35">
          ...
          <p>
            Session management will be available with full tutor tools.<br />Your profile is being
            reviewed for platform access.
          </p>
          <a href="#features" class="inline-block mt-4 text-gold hover:underline text-sm"
            >Learn about tutor features →</a
          >
        </div>
      </div>
    {/if}
