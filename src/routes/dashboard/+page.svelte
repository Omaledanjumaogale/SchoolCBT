...
<script lang="ts">
  ...
  function switchTab(tab: string) { activeTab = tab; }
  ...
  const tutors = [
    { initials:'AB', name:'Adewale Bello',   subjects:'Math · Physics', rating:4.9, students:124, rate:'₦750/hr', color:'cobalt-light/60' },
    { initials:'CN', name:'Chiamaka Nwosu',  subjects:'Chem · Biology', rating:4.8, students:98,  rate:'₦900/hr', color:'jade/40' },
    { initials:'YD', name:'Yakubu Dangote',  subjects:'English · Govt', rating:4.6, students:76,  rate:'₦650/hr', color:'scarlet/40' }
  ];
  ...
  <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    {#if activeTab === 'overview'}
      ...
      <div class="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <div class="stat-card p-5 text-center">
          <div class="mb-1 font-mono text-[10px] text-white/35 uppercase tracking-widest">Time Spent</div>
          <div class="font-sora text-3xl font-bold text-white">42h 18m</div>
          <div class="badge badge-blue mt-2">↑ 6h this week</div>
        </div>
        <div class="stat-card p-5 text-center">
          <div class="mb-1 font-mono text-[10px] text-white/35 uppercase tracking-widest">Questions</div>
          <div class="font-sora text-gold text-3xl font-bold">1,850</div>
          <div class="badge badge-gold mt-2">73% correct</div>
        </div>
        <div class="stat-card p-5 text-center">
          <div class="mb-1 font-mono text-[10px] text-white/35 uppercase tracking-widest">Batches</div>
          <div class="font-sora text-3xl font-bold text-white">37</div>
          <div class="badge badge-blue mt-2">12 with ≥75%</div>
        </div>
        <div class="stat-card p-5 text-center">
          <div class="mb-1 font-mono text-[10px] text-white/35 uppercase tracking-widest">Pass Prob.</div>
          <div class="font-sora text-jade text-3xl font-bold">87%</div>
          <div class="badge badge-jade mt-2">↑ 4% this month</div>
        </div>
      </div>
      ...
      <div class="grid gap-5 lg:grid-cols-3">
        ...
        <div class="glass-card p-6">
          ...
          <div class="space-y-3">
            {#each [
              { icon:'🥇', title:'Gold — Mathematics', sub:'Batch 35 · 92% score' },
              { icon:'🥈', title:'Silver — Physics',   sub:'Batch 33 · 82% score' },
              { icon:'🏆', title:'7-Day Streak!',      sub:'Consistent daily practice' },
              { icon:'⭐', title:'Top 10% National',   sub:'Mathematics ranking' }
            ] as award}
    ...
    {:else if activeTab === 'analytics'}
      <div class="grid gap-5 md:grid-cols-2">
        <div class="glass-card p-6">
          ...
          <div class="space-y-3">
            {#each [
              {topic:'Number Theory',  pct:94, c:'jade'},
              {topic:'Algebra & Sets', pct:87, c:'jade'},
              {topic:'Trigonometry',   pct:72, c:'gold'},
              {topic:'Geometry',       pct:58, c:'scarlet'},
              {topic:'Statistics',     pct:80, c:'jade'},
              {topic:'Calculus',       pct:52, c:'scarlet'}
            ] as t}
              <div class="flex items-center gap-3">
                <span class="text-xs text-white/50 w-28 shrink-0">{t.topic}</span>
                <div class="pbar flex-1"><div class="pfill bg-{t.c}" style="width:{t.pct}%"></div></div>
        ...
        <div class="glass-card p-6">
          ...
          <div class="space-y-3">
            {#each [
              {level:"Knowledge & Recall",        pct:92, cls:'jade'},
              {level:"Comprehension",              pct:85, cls:'jade'},
              {level:"Application",                pct:71, cls:'gold'},
              {level:"Analysis & Synthesis",       pct:55, cls:'scarlet'}
            ] as b}
              <div class="flex items-center gap-3 p-3 rounded-xl bg-{b.cls}/10 border border-{b.cls}/20">
                <div class="flex-1">
                  <div class="text-sm text-white/80 mb-1">{b.level}</div>
                  <div class="pbar"><div class="pfill bg-{b.cls}" style="width:{b.pct}%"></div></div>
                </div>
    ...
    {:else if activeTab === 'study'}
      <div class="glass-card p-6">
        ...
        <div class="space-y-3">
          {#each [
            {week:'W1', topic:'Number Theory & Sets',          done:100, color:'jade',  status:'Done ✓'},
            {week:'W2', topic:'Algebra, Equations & Inequalities', done:100, color:'jade', status:'Done ✓'},
            {week:'W3', topic:'Trigonometry',                  done:65,  color:'gold',  status:'65%'},
            {week:'W4', topic:'Geometry & Mensuration',        done:0,   color:'white/20', status:'Locked'},
            {week:'W5', topic:'Statistics & Probability',      done:0,   color:'white/20', status:'Locked'}
          ] as p}
            <div class="flex items-center gap-4 p-4 rounded-xl
                        {p.done === 100 ? 'bg-jade/10 border border-jade/20' :
                         p.done > 0     ? 'bg-gold/10 border border-gold/20' :
                                          'bg-cobalt/30 border border-white/06'}">
              <div class="flex h-10 w-10 items-center justify-center rounded-xl text-sm font-bold shrink-0
                          {p.done > 0 ? 'bg-jade/20 text-jade' : 'bg-cobalt/40 text-white/40'}">{p.week}</div>
              <div class="flex-1">
                <div class="text-sm font-medium {p.done > 0 ? 'text-white' : 'text-white/50'}">{p.topic}</div>
                <div class="pbar mt-2"><div class="pfill bg-{p.color}" style="width:{p.done}%"></div></div>
              </div>
              <span class="text-xs font-bold shrink-0
                           {p.done === 100 ? 'text-jade' : p.done > 0 ? 'text-gold' : 'text-white/25'}">{p.status}</span>
            </div>
          {/each}
        </div>
      </div>
    ...
    {:else if activeTab === 'reports'}
      <div class="glass-card overflow-hidden">
        <div class="flex items-center justify-between p-5 border-b border-white/06">
          ...
          <span class="badge badge-blue">37 Reports</span>
        </div>
    ...
    {:else if activeTab === 'tutors'}
      <div class="glass-card p-6">
        ...
        <div class="grid gap-4 md:grid-cols-3">
          {#each tutors as t}
            <div class="bg-cobalt/30 rounded-xl p-4 border border-white/06 hover:border-gold/20 transition-all">
              <div class="flex items-center gap-3 mb-3">
                <div class="avatar h-10 w-10 bg-{t.color} text-white text-sm border border-cobalt-xdark">{t.initials}</div>
                <div>
