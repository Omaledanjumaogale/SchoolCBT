...
<script lang="ts">
  ...
  onMount(async () => {
    if (!$auth) { goto('/'); return; }
    else if ($role === 'tutor') { goto('/tutor'); return; }
  ...
  const recentBatches = sessions.length > 0 ? sessions.map((s: any, i: number) => ({
      ...
      {#if activeTab === 'overview'}
        <div class="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <div class="stat-card p-5 text-center">
            ...
            <div class="font-sora text-gold text-3xl font-bold">{totalQuestions.toLocaleString()}</div>
            ...
            <div class="space-y-4">
              {#each subjects as s}
                <div>
                  ...
                  <div class="pbar"><div class="pfill bg-{s.color}" style="width:{s.pct}%"></div></div>
                </div>
              {/each}
            </div>
          </div>
          ...
          <div class="space-y-3">
            {#each [
              ...
              <div class="flex items-center gap-4 p-4 rounded-xl
                          {p.done === 100 ? 'bg-jade/10 border border-jade/20' : p.done > 0 ? 'bg-gold/10 border border-gold/20' : 'bg-cobalt/30 border border-white/06'}">
                ...
                <span class="text-xs font-bold shrink-0 {p.done === 100 ? 'text-jade' : p.done > 0 ? 'text-gold' : 'text-white/25'}">{p.status}</span>
              </div>
            {/each}
          </div>
        </div>
