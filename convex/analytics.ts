  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      ...
      const newAvgScore = Math.round(
        (existing.avgScore * existing.totalBatches + pct) / newTotalBatches,
      )
      const newStreak = now - existing.lastActiveAt < 86400000 * 2 ? existing.streakDays + 1 : 1

      const subjectBreakdown =
        (existing.subjectBreakdown as Record<
          string,
          { pct: number; correct: number; total: number }
        >) ?? {}
...
export const getLeaderboard = query({
  ...
  handler: async (ctx, args) => {
    ...
    return all.sort((a, b) => b.avgScore - a.avgScore).slice(0, limit)
  },
})
